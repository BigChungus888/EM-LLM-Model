...
from .chunk import ChunkMLPUpdate
from .naive import mlp_chunk
...
class SwiGLUSGD(nn.Module):
    ...
    def forward(
        ...
        initial_weights = self._get_initial_weights(past_key_values)
        if self.mode == "chunk":
    ...
    def _get_initial_weights(self, past_key_values):
        if past_key_values is None:
            return None
        if isinstance(past_key_values, tuple):
            return past_key_values[0]
        if isinstance(past_key_values, list):
            return past_key_values[0] if len(past_key_values) > 0 else None
        if self.layer_idx is not None and hasattr(past_key_values, "__len__") and hasattr(past_key_values, "__getitem__"):
            try:
