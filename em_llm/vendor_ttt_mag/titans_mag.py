...
# use the vendored mask helper so MAG fusion can be wired into EM-LLM without
# importing utility code from the donor repository at runtime.
from .titans_utils import create_mag_block_mask
...
class MemoryAsGate(nn.Module):
    def __init__(
        ...
        # keep the donor callback contract intact for the vendored memory module.
    ...
    def get_qkv(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # the 340M config still performs local mixing with short convolutions even though
        # `use_attention=false`, so this is where token-local context enters the memory rule.
        hidden_states_conv, _ = self.input_conv(x)
        hidden_states = x + hidden_states_conv
        q = q_proj_layer(hidden_states)
        q_conv, _ = self.q_conv1d(q)
        q = q + q_conv
        k = k_proj_layer(hidden_states)
        k_conv, _ = self.k_conv1d(k)
        k = k + k_conv
        v = v_proj_layer(hidden_states)
        v_conv, _ = self.v_conv1d(v)
        v = v + v_conv

        # normalize q and k before the linear memory update for numerical stability.
    ...
    def forward(
        ...
        **kwargs
        ...
        # the active 340M path delegates directly to LinearSGD; `past_key_values` is
        # passed through the interface, but the current linear memory path rebuilds state from `x`.
        atlas_out, new_state = self.M(x, past_key_values)
        if not self.use_attention:
            # the 340M config exits here because `use_attention=false`.
