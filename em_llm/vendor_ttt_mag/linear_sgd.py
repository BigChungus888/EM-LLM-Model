...
This vendored copy is the only place where EM-LLM-specific fixes should land.
The donor repository remains unchanged so the hybrid work can evolve in isolation.
"""
...
# use the vendored kernel helper so this subtree stays self-contained inside EM-LLM.
from .naive import linear_chunk
...
class LinearSGD(nn.Module):
    ...
    def forward(
        self,
        inputs: torch.Tensor,
        past_key_values: Cache | Tuple[torch.Tensor, Optional[torch.Tensor]] | None = None
    ) -> Tuple[
        ...
        K = self.head_k_dim
        V = self.head_v_dim
        # `MemoryAsGate.get_qkv` owns the projection logic, so the inner update rule
        # stays agnostic to how q, k, and v were produced.
        q, k, v = self._get_qkv(inputs)  # [B, T, internal_dim]
        q, k, v = self.split_heads(q), self.split_heads(k), self.split_heads(v)
        # `eta` is the per-token decay term and `theta` is the per-token learning rate.
        eta = self.compute_weight_decay(inputs)  # [B, H, T]
        theta = self.compute_learning_rate(inputs)  # [B, H, T]
        # recover the carried fast weights before running the next chunk so the
        # vendored copy behaves like one continuous recurrent model.
        ...
        if self.mode == "chunk":
            # the 340M config uses chunk mode with chunk_size=64.
            dynamic_weights, outputs = self.chunk_linear_rule(
                q, k, v, eta, theta, initial_weights, B, H, K, T, V, BT
            )
        else:
            dynamic_weights, outputs = linear_chunk(
                q, k, v, eta, theta, initial_weights, B, H, K, T, V, BT
            )
        if self.use_gate:
            # `memory_model_config.use_gate=true` enables value-space gating before projection back to hidden size.
    ...
    def _get_initial_weights(
        ...
        state = None
        # support the cache shapes that the vendored tests use today and the
        # layer-local cache payloads the EM-LLM hybrid path will use later.
        if past_key_values is None:
            pass
        elif isinstance(past_key_values, tuple):
            state = past_key_values[0]
        elif isinstance(past_key_values, list):
            state = past_key_values[0] if len(past_key_values) > 0 else None
        elif self.layer_idx is not None and hasattr(past_key_values, "__len__") and hasattr(past_key_values, "__getitem__"):
            try:
                layer_state = past_key_values[self.layer_idx]
            except (IndexError, KeyError, TypeError):
                layer_state = None
            if isinstance(layer_state, tuple):
                # mirror the `(dynamic_weights, aux_state)` payload returned by `forward`.
                state = layer_state[0]
            else:
                state = layer_state

        if state is None:
            return torch.zeros(B, H, V, K, device=device, dtype=dtype)
        # cast the restored state onto the active device because EM-LLM may move
        # cache payloads across devices as part of long-context execution.
    ...
    def chunk_linear_rule(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, eta: torch.Tensor,
                          theta: torch.Tensor, initial_weights: torch.Tensor, B: int, H: int, K: int, T: int, V: int, BT: int):
        NT = (T + BT - 1) // BT
        # start from the carried recurrent state instead of reinitializing to
        # zeros so prompt chunks and decode steps share one continuous memory.
        dynamic_weights = initial_weights.clone()
        outputs = torch.empty(v.shape, device=q.device, dtype=q.dtype)
        q = q * K ** (-1 / 2)

        # carry the learned weights chunk by chunk so the last state can be returned as cache.
        for idx in range(NT):
            seg_start = idx * BT
